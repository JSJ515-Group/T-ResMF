...
class SGL(GraphProPluginModel):
    ...
    def cal_loss(self, batch_data):
        # 兼容 GraphPro 的 dataloader (users, pos, neg, extra)
        ...
        # 先做一轮 edge dropout，得到 base 图（同时用于 BPR + CL）
        ...
        # 对比学习损失
        cl_loss = self.lbd * self.cal_cl_loss(
            [users, pos_items],
            edges,
            edge_norm,
    ...
    def cal_cl_loss(self, idx, edges, edge_norm, edge_times=None):
        """
        idx: [users, pos_items]
        edges, edge_norm, edge_times: 当前这一轮 pretrain 用到的图
        这里按照 SGL 的思想，用两次 edge dropout 构造两个增强视图
        """

        # 1) 取出当前 batch 里出现过的用户 / 正样本 item 索引
        if isinstance(idx[0], torch.Tensor):
            u_idx = torch.unique(idx[0]).long().to(args.device)
        else:
            u_idx = torch.unique(torch.LongTensor(idx[0]).to(args.device))

        if isinstance(idx[1], torch.Tensor):
            i_idx = torch.unique(idx[1]).long().to(args.device)
        else:
            i_idx = torch.unique(torch.LongTensor(idx[1]).to(args.device))

        # 2) 构造 view1（edge dropout 一次）
        edges1, mask1 = self.edge_dropout(edges, 0.5, return_mask=True)
        edge_norm1 = edge_norm[mask1]
        if edge_times is not None:
            edge_times1 = edge_times[mask1]
        else:
            edge_times1 = None

        # 3) 构造 view2（edge dropout 再来一次，得到另一份增强图）
        edges2, mask2 = self.edge_dropout(edges, 0.5, return_mask=True)
        edge_norm2 = edge_norm[mask2]
        if edge_times is not None:
            edge_times2 = edge_times[mask2]
        else:
            edge_times2 = None

        # 4) 在两份增强图上各自跑一遍 GCN（这里 forward 没有 perturbed 参数）
        user_view_1, item_view_1 = self.forward(edges1, edge_norm1, edge_times=edge_times1)
        user_view_2, item_view_2 = self.forward(edges2, edge_norm2, edge_times=edge_times2)

        # 5) 用温度 self.temp 做 InfoNCE（SGL 的对比学习）
        user_cl_loss = cal_infonce(user_view_1[u_idx], user_view_2[u_idx], self.temp)
        item_cl_loss = cal_infonce(item_view_1[i_idx], item_view_2[i_idx], self.temp)
